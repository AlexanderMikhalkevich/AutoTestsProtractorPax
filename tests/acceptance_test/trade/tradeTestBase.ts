...
export async function checkMoreInfoPanelElements(userNick: string): Promise<void> {
    ...
    expect(await moreInfo.elements.trustedBy.getText()).toMatch(/^Trusted by \d people$/);
    expect(await moreInfo.elements.blockedBy.getText()).toMatch(/^Blocked by \d people$/);
